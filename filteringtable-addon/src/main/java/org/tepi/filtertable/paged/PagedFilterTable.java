...
public class PagedFilterTable<T extends Container.Indexed & Container.Filterable & Container.ItemSetChangeNotifier>
	...
	public HorizontalLayout createControls(PagedFilterControlConfig config) {
		...
		itemsPerPageSelect.addValueChangeListener(e -> setPageLength((Integer) e.getProperty().getValue()));
		...
		currentPageTextField.addValueChangeListener(e -> {
		...
		final Button first = new Button(config.getFirst(), e -> setCurrentPage(0));
		final Button previous = new Button(config.getPrevious(), e -> previousPage());
		final Button next = new Button(config.getNext(), e -> nextPage());
		final Button last = new Button(config.getLast(), e -> setCurrentPage(getTotalAmountOfPages()));
	...
	public void setContainerDataSource(Container newDataSource) {
		...
		PagedFilterTableContainer<T> pagedFilteringTableContainer = new PagedFilterTableContainer<T>((T) newDataSource);
